...
source('00_Functions.R')
  ...
  ################################################################################################################################
  ##### Compute each of the three DR estimators on observed data, with 2 variance estimates (influence function and sandwich)#####
  ################################################################################################################################
  ...
  DR.PI2<-as.data.frame(cbind(DR.PI,IFseATE.PI))
  colnames(DR.PI2)<-c('ATE', 'ESseATE','IFseATE')
  DR.PI2$type<-'PI,CS'
  ...
  DR.WTD2<-as.data.frame(cbind(DR.WTD,IFseATE.WTD))
  colnames(DR.WTD2) <-c('ATE', 'ESseATE','IFseATE')
  DR.WTD2$type<-'WTD,CS'
  ...
  simdat1$Y_scaled<-ifelse(simdat1$Y_scaled<0.0005,0.0005,ifelse(simdat1$Y_scaled>0.9995,0.9995,simdat1$Y_scaled))
  ...
  DR.TMLE2<-as.data.frame(cbind(DR.TMLE,IFseATE.TMLE))
  colnames(DR.TMLE2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.PI.mis2<-as.data.frame(cbind(DR.PI.mis,IFseATE.PI.mis))
  colnames(DR.PI.mis2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.WTD.mis2<-as.data.frame(cbind(DR.WTD.mis,IFseATE.WTD.mis))
  colnames(DR.WTD.mis2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.TMLE.mis2<-as.data.frame(cbind(DR.TMLE.mis,IFseATE.TMLE.mis))
  colnames(DR.TMLE.mis2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.PI.mw2<-as.data.frame(cbind(DR.PI.mw,IFseATE.PI.mw))
  colnames(DR.PI.mw2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  #fit outcome model (correct model but wrong weights)
  ...
  DR.WTD.mw2<-as.data.frame(cbind(DR.WTD.mw,IFseATE.WTD.mw))
  colnames(DR.WTD.mw2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.TMLE.mw2<-as.data.frame(cbind(DR.TMLE.mw,IFseATE.TMLE.mw))
  colnames(DR.TMLE.mw2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.PI.mb2<-as.data.frame(cbind(DR.PI.mb,IFseATE.PI.mb))
  colnames(DR.PI.mb2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.WTD.mb2<-as.data.frame(cbind(DR.WTD.mb,IFseATE.WTD.mb))
  colnames(DR.WTD.mb2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  DR.TMLE.mb2<-as.data.frame(cbind(DR.TMLE.mb,IFseATE.TMLE.mb))
  colnames(DR.TMLE.mb2)<-c('ATE', 'ESseATE','IFseATE')
  ...
  colnames(DR.all2)<-c('ATE', 'ESseATE','IFseATE','type','sim')
