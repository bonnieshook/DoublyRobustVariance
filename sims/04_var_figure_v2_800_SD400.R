...
calc_table <- function(ACE,ESse,IFse){
  ...
  results<-c(ESE, ASE.ES, SER.ES, ASE.boot, SER.boot, ASE.IF, SER.IF)
}

#calculate metrics for each estimator and specification
PI.bc<-c("PI,CS", "PI", calc_table(res.PI.CS$ATE,res.PI.CS$ESseATE,res.PI.CS$IFseATE,res.PI.CS$bootseATE))
WTD.bc<-c("WTD,CS", "Wtd Reg",calc_table(res.WTD.CS$ATE,res.WTD.CS$ESseATE,res.WTD.CS$IFseATE,res.WTD.CS$bootseATE))
TMLE.bc<-c("TMLE,CS", "TMLE",calc_table(res.TMLE.CS$ATE,res.TMLE.CS$ESseATE,res.TMLE.CS$IFseATE,res.TMLE.CS$bootseATE))
all.cor<-rbind(PI.bc,WTD.bc,TMLE.bc)

PI.mo<-c("PI,MO", "PI",calc_table(res.PI.MO$ATE,res.PI.MO$ESseATE,res.PI.MO$IFseATE,res.PI.MO$bootseATE))
WTD.mo<-c("WTD,MO", "Wtd Reg",calc_table(res.WTD.MO$ATE,res.WTD.MO$ESseATE,res.WTD.MO$IFseATE,res.WTD.MO$bootseATE))
TMLE.mo<-c("TMLE,MO", "TMLE",calc_table(res.TMLE.MO$ATE,res.TMLE.MO$ESseATE,res.TMLE.MO$IFseATE,res.TMLE.MO$bootseATE))
all.mo<-rbind(PI.mo,WTD.mo,TMLE.mo)

PI.MW<-c("PI,MW", "PI",calc_table(res.PI.MW$ATE,res.PI.MW$ESseATE,res.PI.MW$IFseATE,res.PI.MW$bootseATE))
WTD.MW<-c("WTD,MW", "Wtd Reg",calc_table(res.WTD.MW$ATE,res.WTD.MW$ESseATE,res.WTD.MW$IFseATE,res.WTD.MW$bootseATE))
TMLE.MW<-c("TMLE,MW", "TMLE",calc_table(res.TMLE.MW$ATE,res.TMLE.MW$ESseATE,res.TMLE.MW$IFseATE,res.TMLE.MW$bootseATE))
all.mw<-rbind(PI.MW,WTD.MW,TMLE.MW)

PI.MB<-c("PI,MB", "PI",calc_table(res.PI.MB$ATE,res.PI.MB$ESseATE,res.PI.MB$IFseATE,res.PI.MB$bootseATE))
WTD.MB<-c("WTD,MB", "Wtd Reg",calc_table(res.WTD.MB$ATE,res.WTD.MB$ESseATE,res.WTD.MB$IFseATE,res.WTD.MB$bootseATE))
TMLE.MB<-c("TMLE,MB", "TMLE",calc_table(res.TMLE.MB$ATE,res.TMLE.MB$ESseATE,res.TMLE.MB$IFseATE,res.TMLE.MB$bootseATE))
all.mb<-rbind(PI.MB,WTD.MB,TMLE.MB)


#combine and format
all<-as.data.frame(rbind(all.cor,all.mo,all.mw,all.mb))
colnames(all)<-c('type','estname','ESE', 'ASE.ES', 'SER.ES','ASE.boot', 'SER.boot', 'ASE.IF', 'SER.IF')

data<-merge(res, all, by="type", all.x=TRUE)
data$sd_ratio_ES<-data$ESseATE/as.numeric(data$ESE)
data$sd_ratio_boot<-data$bootseATE/as.numeric(data$ESE)
...
boot<-data[,c('type','estname','bootseATE','ESE','ASE.boot','SER.boot')]
boot$EST<-"Nonparametric Bootstrap"
colnames(boot)<-c('type','estname','seATE','ESE','ASE','SER','EST')
...
stack<-as.data.frame(rbind(sand,boot,IF))
stack$std_ratio<-stack$seATE/as.numeric(stack$ESE)
stack$estname2<-ifelse(stack$estname=="PI","Classic",stack$estname)
stack$Scenario<-factor(stack$estname2,levels=c('Classic','Wtd Reg','TMLE'))
stack$EST2<-factor(stack$EST,levels = c("Empirical Sandwich", "Nonparametric Bootstrap", "Influence Function"))
...
var.plot.CS<-ggplot(CS,aes(x=Scenario, y=std_ratio, fill=EST2))+ scale_y_continuous(limits=c(0.5,11),breaks=c(0.5,0.75,1,1.25,1.5,1.75,2,2.5,3))+geom_violin() + scale_fill_manual(values = c("#FF3D3D","#57B0FF","forestgreen"))+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + theme(legend.position="bottom",legend.title = element_blank())+xlab("Both Correct")+ylab("")+geom_vline(xintercept=c(1.5,2.5), colour='light grey',lwd=1)+ geom_hline(yintercept=1.0, linetype="dashed", color = "black")

legend <- get_plot_component(var.plot.CS, 'guide-box-bottom', return_all = TRUE)
