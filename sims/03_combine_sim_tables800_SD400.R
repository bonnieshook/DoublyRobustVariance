...
calc_table <- function(ACE,ESse,IFse){
  ...
  results<-c(round(bias,digits=1),"&",round(relbias,digits=1),"&", round(ESE, digits=1),"&", round(ASE.ES, digits=1),"&", round(SER.ES, digits=2),"&", round(cov.ES, digits=0),"&",round(ASE.IF, digits=1),"&", round(SER.IF, digits=2),"&", round(cov.IF, digits=0),'\\')
}

#format results for LaTeX table
PI.bc<-c("CS", "&", "PI", "&", calc_table(res.PI.CS$ATE,res.PI.CS$ESseATE,res.PI.CS$IFseATE))
WTD.bc<-c("", "&","WTD", "&", calc_table(res.WTD.CS$ATE,res.WTD.CS$ESseATE,res.WTD.CS$IFseATE))
TMLE.bc<-c("", "&","TMLE", "&", calc_table(res.TMLE.CS$ATE,res.TMLE.CS$ESseATE,res.TMLE.CS$IFseATE))
all.cor<-rbind(PI.bc,WTD.bc,TMLE.bc)

PI.mo<-c("MO", "&","PI", "&", calc_table(res.PI.MO$ATE,res.PI.MO$ESseATE,res.PI.MO$IFseATE))
WTD.mo<-c("", "&", "WTD", "&", calc_table(res.WTD.MO$ATE,res.WTD.MO$ESseATE,res.WTD.MO$IFseATE))
TMLE.mo<-c("","&", "TMLE", "&", calc_table(res.TMLE.MO$ATE,res.TMLE.MO$ESseATE,res.TMLE.MO$IFseATE))
all.mo<-rbind(PI.mo,WTD.mo,TMLE.mo)

PI.MW<-c("MW", "&","PI", "&", calc_table(res.PI.MW$ATE,res.PI.MW$ESseATE,res.PI.MW$IFseATE))
WTD.MW<-c("", "&", "WTD", "&", calc_table(res.WTD.MW$ATE,res.WTD.MW$ESseATE,res.WTD.MW$IFseATE))
TMLE.MW<-c("", "&", "TMLE", "&", calc_table(res.TMLE.MW$ATE,res.TMLE.MW$ESseATE,res.TMLE.MW$IFseATE))
all.mw<-rbind(PI.MW,WTD.MW,TMLE.MW)

PI.MB<-c("MB", "&","PI", "&", calc_table(res.PI.MB$ATE,res.PI.MB$ESseATE,res.PI.MB$IFseATE))
WTD.MB<-c("", "&", "WTD", "&", calc_table(res.WTD.MB$ATE,res.WTD.MB$ESseATE,res.WTD.MB$IFseATE))
TMLE.MB<-c("", "&", "TMLE", "&", calc_table(res.TMLE.MB$ATE,res.TMLE.MB$ESseATE,res.TMLE.MB$IFseATE))
all.mb<-rbind(PI.MB,WTD.MB,TMLE.MB)


#output
all<-rbind(all.cor,all.mo,all.mw,all.mb)
write.csv(all, "SimSummary_n800_SD400_041924.csv")
